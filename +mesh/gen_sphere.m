function [vv,ff] = gen_sphere(varargin)
...
%
%
%   EDIT: The subdivision code has been vectorized to make the function
%   more efficient.
%   Edited by Filippo Maggioli, 'La Sapienza', Department of Computer
%   Science.
...
%         f_(idx,:) = nfc;
%     end
    % Compute mid points and assign them new indices
    M12 = (v(f(:, 1), :) + v(f(:, 2), :)) ./ 2;
%     M12 = normr(M12);
    M12 = M12 ./ vecnorm(M12, 2, 2);
    I12 = (1:size(M12, 1))' + size(v, 1);
    M23 = (v(f(:, 2), :) + v(f(:, 3), :)) ./ 2;
%     M23 = normr(M23);
    M23 = M23 ./ vecnorm(M23, 2, 2);
    I23 = (1:size(M23, 1))' + size(I12, 1) + size(v, 1);
    M31 = (v(f(:, 3), :) + v(f(:, 1), :)) ./ 2;
%     M31 = normr(M31);
    M31 = M31 ./ vecnorm(M31, 2, 2);
    I31 = (1:size(M31, 1))' + size(I23, 1) + size(I12, 1) + size(v, 1);
