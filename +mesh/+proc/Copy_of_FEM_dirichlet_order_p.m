%% TODO
% Order one does not compute boundary conditions, it calls cotangent method
% with Newmann conditions.

function [S, A, Al] = FEM_dirichlet_order_p(M, p, ICache)
%[S, A] = FEM_ORDER_P(M, p) Compute the stiffness (S) and mass (A) matrices
%for a order p Finite Element discretization of Laplace-Beltrami operator
%on a mesh M.
%
%[S, A, Al] = FEM_ORDER_P(M, p) Also compute the lumped mass matrix. Throws
%a warning if the lumped mass results singular.
%
%[--] = FEM_ORDER_P(M, p, ICache) Since the computation of the I# matrices
%is costly, the function caches them on disk after the first time they are
%computed. The optional parameter ICache tells to the function the
...
%saved in that directory in a single .mat file named "OrderP.mat".
    ...
    if p == 1
        if nargout > 2
            [S, A, Al] = cotangent_method(M);
        else
            [S, A] = cotangent_method(M);
        end
        return;
    end
    
    % Compute the adjacency matrix and the matrices entries
    ...
    % Compute the indices to transfer local mass/stiffness matrices to
...
function [W, Sc, Sl] = cotangent_method(M)
    % Stiffness (p.s.d.)
    angles = zeros(M.m,3);
    for i=1:3
        a = mod(i-1,3)+1;
