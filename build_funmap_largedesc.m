function C = build_funmap_largedesc(Src, Trg, KSrc, KTrg, SrcLM, TrgLM)
...
% adapted to a function by Filippo Maggioli.
    ...
    if nargin == 4
        SrcLM = sort(euclidean_fps(Src, 10));
%         SrcLM = (500:1000:6000)';
        TrgLM = SrcLM;
    end
    LM = [SrcLM, TrgLM];
    ...
    % Compute descriptors
    FCTSrc = [];
    FCTSrc = [FCTSrc, waveKernelSignature(Src.Phi(:, 1:200), Src.Lambda(1:200), Src.A, 200)];
    FCTSrc = [FCTSrc, waveKernelMap(Src.Phi(:, 1:200), Src.Lambda(1:200), Src.A, 200, LM(:, 1))];
    FCTSrc = FCTSrc(:,1:10:end);

    FCTTrg = [];
    FCTTrg = [FCTTrg, waveKernelSignature(Trg.Phi(:, 1:200), Trg.Lambda(1:200), Trg.A, 200)];
    FCTTrg = [FCTTrg, waveKernelMap(Trg.Phi(:, 1:200), Trg.Lambda(1:200), Trg.A, 200, LM(:, 2))];
