...
%% Parameters
SrcFile = 'Meshes/tr_reg/tr_reg_019.ply';
TrgFile = 'Meshes/tr_reg/tr_reg_094.ply';
KSrc = 15;
KTrg = 15;
...
SrcPlot = mesh.transform.rotate(Src, 'x', 90);
% Src = mesh.transform.normalize(Src);
Trg = mesh.init(TrgFile);
TrgPlot = mesh.transform.rotate(Trg, 'x', 90);
...
Phi = Src.Phi(:, 5);
ScaledPhi = (Phi - min(Phi)) ./ (max(Phi) - min(Phi));
f = cos(8 .* pi .* ScaledPhi);
MinF = min(f);
MaxF = max(f);
LenF = MaxF - MinF;
Lim = max(abs(f));
...
subplot(2, 4, 1);
utils.plot_scalar_map(SrcPlot, f, utils.cmaps.bwr, true);
if PlotTitles
    title('Source', ...
          'Interpreter', 'LaTeX', ...
          'FontSize', FontSize);
...
subplot(2, 4, 2);
utils.plot_scalar_map(TrgPlot, f, utils.cmaps.bwr, true);
if PlotTitles
    title('Ground Truth', ...
          'Interpreter', 'LaTeX', ...
          'FontSize', FontSize);
...
subplot(2, 4, 3);
utils.plot_scalar_map(TrgPlot, FTran, utils.cmaps.bwr, true);
if PlotTitles
    TitleString = {"$NK$ Eigs";
