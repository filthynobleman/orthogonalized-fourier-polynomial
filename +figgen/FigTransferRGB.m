% Plots the visual transfer and the numerical error for a RGB signal
...
ColMeshFile = 'Meshes/SHREC19_weared.ply';
KSrc = 60;
KTrg = 50;
...
Src = mesh.transform.rotate(Src, 'z', -105);
...
Colored = mesh.init(ColMeshFile);
Colored = mesh.transform.rotate(Colored, 'z', -105);
Colored = mesh.transform.normalize(Colored);
% Search for the correspondence to solve problems with vertex ordering
T12 = flann_search(Colored.VERT', Src.VERT', 1, struct());
clear Colored;
% Load the colors
[Data, ~] = mesh.io.ply_read(ColMeshFile);
f = Data.vertex;
f = [f.red, f.green, f.blue];
f = f ./ 256;
clear Data;
% Reorder the colors
f = f(T12, :);
...
subplot(2, 4, 2);
utils.plot_rgb_map(Trg, f(Src.GT, :), true);
if PlotTitles
    title('Ground Truth', ...
          'Interpreter', 'LaTeX', ...
          'FontSize', FontSize);
...
Diff = f(Src.GT, :) - FTran;
ErrNK = sqrt(sum(diag(Diff' * Trg.A * Diff) ./ diag(sqrt(f(Src.GT, :)' * Trg.A * f(Src.GT, :)))));

subplot(2, 4, 3);
utils.plot_rgb_map(Trg, FTran, true);
if PlotTitles
    TitleString = {"$NK$ Eigs";
...
Diff = f(Src.GT, :) - FTran;
ErrK = sqrt(sum(diag(Diff' * Trg.A * Diff) ./ diag(sqrt(f(Src.GT, :)' * Trg.A * f(Src.GT, :)))));
...
% FTran = (((FTran - min(FTran)) ./ (max(FTran) - min(FTran))) .* LenF) + MinF;
Diff = f(Src.GT, :) - FTran;
ErrPoly = sqrt(sum(diag(Diff' * Trg.A * Diff) ./ diag(sqrt(f(Src.GT, :)' * Trg.A * f(Src.GT, :)))));
...
% FTran = (((FTran - min(FTran)) ./ (max(FTran) - min(FTran))) .* LenF) + MinF;
Diff = f(Src.GT, :) - FTran;
ErrOrtho = sqrt(sum(diag(Diff' * Trg.A * Diff) ./ diag(sqrt(f(Src.GT, :)' * Trg.A * f(Src.GT, :)))));
...
% FTran = (((FTran - min(FTran)) ./ (max(FTran) - min(FTran))) .* LenF) + MinF;
Diff = f(Src.GT, :) - FTran;
ErrOrthoReiter = sqrt(sum(diag(Diff' * Trg.A * Diff) ./ diag(sqrt(f(Src.GT, :)' * Trg.A * f(Src.GT, :)))));
