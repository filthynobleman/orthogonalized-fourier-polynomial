...
void SHOTDescriptor::getSHOTLocalRF(
      pcl_t& data, int p, double radius,  vec3d<double> & X,  vec3d<double> & Y,  vec3d<double> & Z) const
{
 ...
 *
 */
void SHOTDescriptor::describe(pcl_t& data, int feat_index, shot& desc) const
{
   ...
   for (int j=0; j<n_neighs; ++j)
   {
      ...
      const vec3d<double>& normal = data.get_normal(neighs[j]);
